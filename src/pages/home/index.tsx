  ...
  return (
    <View className={styles.container}>
            ...
            <View className={styles.userInfo}>
              ...
              <View className={styles.stats}>
                <Text className={styles.statsText}>
                  {locale('追踪中: {count}', { count: stats?.activeTrackers || 0 })}
                </Text>
              </View>
            </View>
          )}
      ...
      <View className={styles.main}>
        {/* 输入区域 */}
        <TrackInput onSubmit={handleSubmit} loading={trackerLoading} />

        {/* 功能入口 */}
        <View className={styles.features}>
          <Button className={styles.featureBtn} onClick={handleShowIntro}>
          ...
          <Button className={styles.featureBtn} onClick={handleShowSubscribe}>
            <View className={styles.featureBtnContent}>
              <Text className={styles.featureBtnIcon}>🔔</Text>
          ...
          {isLoggedIn && (
            <Button className={styles.featureBtn} onClick={handleGoToList}>
              <View className={styles.featureBtnContent}>
                <Text className={styles.featureBtnIcon}>📋</Text>
        ...
        <View className={styles.footer}>
          ...
          <Text className={styles.footerSubText}>{locale('及时获取您关注内容的最新动态')}</Text>
        </View>
      </View>
      ...
      <ConfirmDialog
        ...
        title={locale('功能介绍')}
        content={locale(
          '追影小程序可以帮助您追踪微博和小红书的内容更新。只需输入分享链接，即可开始追踪。当内容有更新时，会及时通知您。每次追踪需要消耗10金币，您可以通过观看广告获得金币。'
        )}
      ...
      <ConfirmDialog
        ...
        title={locale('订阅消息提醒')}
        content={locale(
          '开启订阅消息后，当您追踪的内容有更新时，我们会及时通知您。请点击确定开启订阅。'
        )}
